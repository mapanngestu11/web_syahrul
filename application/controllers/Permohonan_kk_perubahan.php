...
class Permohonan_kk_perubahan extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('Front/Permohonan.kk.perubahan.php',$data);
	}

	public function cek_permohonan()
	{
		$kode_permohonan = $this->input->post('kode_permohonan');
		$hasil = $this->M_surat_kk_perubahan->cek_kode_permohonan($kode_permohonan)->result();

		$status = $hasil[0]->status;
		$keterangan = $hasil[0]->keterangan;
		$file_surat = $hasil[0]->file_surat;
        $jenis_permohonan = 'Permohonan KK Perubahan';
        ...
        if ($status == '1') {   
         ...
         redirect('Permohonan_kk_perubahan');
     }
 ...
 public function add()
 {
  ...
  $hasil = $this->M_surat_kk_perubahan->cek_ktp($nik)->result();
